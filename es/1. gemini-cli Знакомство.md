...
**Gemini CLI** es una herramienta que funciona directamente en tu terminal, comprende tu base de código y ayuda a corregir errores mediante consultas en lenguaje natural. Es la respuesta de Google a Claude Code de Anthropic.
Puedes usar Gemini 2.5 Pro (la herramienta cambiará a Gemini 2.5 Flash si se alcanza el límite) y su ventana de contexto de 1 millón de tokens para realizar hasta 60 solicitudes por minuto y 1000 solicitudes por día, todo de forma gratuita.
...
*   **Soporte de documentación:** Puedes crear documentos markdown estructurados, registros de cambios (changelogs) y responder a problemas de GitHub directamente en la terminal.
...
# Inicializar nvm en la sesión actual de la terminal
. "$HOME/.nvm/nvm.sh"
...
> Alternativamente, puedes instalar WSL (Windows Subsystem for Linux) y ejecutar los comandos en Ubuntu o otra distribución de Linux.
...
```
O usa `npm` para la instalación global:
```bash
npm install -g @google/gemini-cli
gemini
```
Después de la instalación, escribe `gemini` en tu terminal para acceder a la herramienta.



#### Paso 2.2: Autenticación
Puedes usar tu cuenta personal de Google para la autenticación. Esto te dará hasta 60 solicitudes por minuto y 1000 solicitudes por día al usar Gemini.
![First Run](assets/gemini_cli_1/auth.png)

En esta guía, utilicé **Iniciar sesión con Google**, pero también puedes usar una **clave API** (establecida como variable de entorno o en un archivo `.env`) o la autenticación de **Vertex AI**.
...
```
Puedes usar el comando `/auth` en el campo de texto para cambiar los métodos de autenticación según sea necesario.
...
Concede permiso para escribir archivos:
...
*   **Comunicación:** Los agentes se comunican entre sí a través de una API RESTful de FastAPI.
*   **Componentes compartidos:** `shared/schemas.py` define estructuras de datos comunes.

Esto te ayudará a navegar sin leer cada archivo manualmente.

#### Analizando y corrigiendo un problema de GitHub
Exploremos algunos problemas abiertos del repositorio de GitHub.

**Prompt:** `Aquí hay un problema de GitHub: [@search https://github.com/AashiDutt/Google-Agent-Development-Kit-Demo/issues/1]. Analiza la base de código y propón un plan de solución de 3 pasos. ¿Qué archivos/funciones deben cambiarse?`
...
Ahora que se han aplicado las correcciones, resumamos los cambios y escribámoslos en Markdown en un archivo `.txt`.

**Prompt:** `Escribe un resumen en markdown del error, la corrección y la cobertura de la prueba. Formatea como una entrada de registro de cambios bajo la versión "v0.2.0".`

Luego, para guardar el resumen en un documento, utilicé el siguiente prompt:
...
Esta sección amplía experimentos anteriores donde exploro cómo Gemini CLI utiliza el **Protocolo de Contexto del Modelo (MCP)** para mantener resúmenes a nivel de archivo e historial de tareas entre prompts. Esto le da a Gemini "memoria de trabajo" dentro de una sesión.
...
Al llamar al comando `/tools` en Gemini CLI, se mostrará una lista de herramientas disponibles que se pueden usar para realizar varias tareas, como editar código, generar pruebas, crear documentación y más.
![Tools](assets/gemini_cli_1/tools.png)

**ReadFolder (ls)**
Enumera archivos y carpetas en un directorio, similar al comando `ls` en la línea de comandos.
...
**Edit (edit)**
Aplica cambios de código usando `diff`. Gemini muestra una vista previa de las ediciones y solicita confirmación antes de aplicarlas.

**WriteFile (write-file)**
Crea nuevos archivos (por ejemplo, `README.md`) con contenido proporcionado por el usuario.
...
Realiza una búsqueda en Google para basar las respuestas en información del mundo real (por ejemplo, para encontrar una explicación para un error).
...
Puedes agregar instrucciones especiales para la IA para un proyecto específico
...
estilos de código y herramientas que el agente debe usar. Esto asegura que el código generado cumpla con los estándares de tu proyecto.
...
Después de eso, escribe `/quit` en Gemini CLI para salir, luego ábrelo de nuevo.
